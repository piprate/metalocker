...
func TestLocalStoreImpl_AssetHead_SetAssetHead(t *testing.T) {
	...
	headID := sharedLocker.HeadID(assetID, "test")

	_, err = dataWallet1.DataStore().AssetHead(headID)
	require.Error(t, err)
	assert.Equal(t, model.ErrAssetHeadNotFound, err)

	// set the asset head to the new data set

	headFuture := sharedLocker.SetAssetHead(assetID, "test", recordID)
	...
	_, err = dataWallet1.DataStore().AssetHead(headID, dataset.FromLocker(sharedLocker.ID()))
	require.NoError(t, err)

	// without FromLocker option

	headDataSet, err := dataWallet1.DataStore().AssetHead(headID)
	require.NoError(t, err)
	...
	headFuture2 := sharedLocker.SetAssetHead(assetID, "test", recordID)
	require.NoError(t, headFuture2.Wait(time.Second*10))

	// run checks

	headDataSet2, err := dataWallet1.DataStore().AssetHead(headID)
	require.NoError(t, err)
	...
	headDataSet3, err := dataWallet2.DataStore().AssetHead(headID)
	require.NoError(t, err)
...
func TestLocalStoreImpl_Submit_SetHeads(t *testing.T) {
	...
	idy, err := dw.NewIdentity(model.AccessLevelManaged, "")
	require.NoError(t, err)

	uniLocker, err := idy.NewLocker("Test Locker")
	require.NoError(t, err)

	// create a data set

	lb, err := uniLocker.NewDataSetBuilder(dataset.WithVault(testbase.TestVaultName), dataset.SetHeads("test"))
	require.NoError(t, err)
	...
	headID := uniLocker.HeadID(f.DataSet().Impression().Asset, "test")

	headDataSet, err := dw.DataStore().AssetHead(headID)
	require.NoError(t, err)
	...
	headDataSet2, err := dw.DataStore().AssetHead(headID)
	require.NoError(t, err)
...
func TestLocalStoreImpl_Submit_Public(t *testing.T) {
	...
	lb, err := uniLocker.NewDataSetBuilder(dataset.WithVault(vaultName), dataset.AsCleartext())
	require.NoError(t, err)
...
func TestLocalStoreImpl_Load(t *testing.T) {
	...
	idy, err := dataWallet.NewIdentity(model.AccessLevelManaged, "")
	require.NoError(t, err)

	// set up a uni-locker

	uniLocker, err := idy.NewLocker("Uni-locker")
	require.NoError(t, err)
	...
	_, err = dataWallet.DataStore().Load(recordID, dataset.FromLocker(uniLocker.ID()))
	require.NoError(t, err)

	// load with FromLocker option (wrong locker)

	_, err = dataWallet.DataStore().Load(recordID, dataset.FromLocker("8XPEnfnmyhb8ZBmU7Ln7QCrR8jZusJvvoZdn9qKCUpND"))
	require.NoError(t, err)

	// load without FromLocker option

	_, err = dataWallet.DataStore().Load(recordID)
	require.NoError(t, err)
}
...
func TestLocalStoreImpl_Load_PublicFromForeignLocker(t *testing.T) {
	...
	ds, err := readerWallet.DataStore().Load(f.ID())
	require.NoError(t, err)
...
func TestLocalStoreImpl_Share(t *testing.T) {
	...
	uniLocker, err := idy1.NewLocker("Uni-locker")
	require.NoError(t, err)
	...
	rootIndex1, err := dataWallet1.CreateRootIndex(testbase.IndexStoreName)
	require.NoError(t, err)

	updater1, err := dataWallet1.IndexUpdater(rootIndex1)
	require.NoError(t, err)
	defer updater1.Close()

	rootIndex2, err := dataWallet2.CreateRootIndex(testbase.IndexStoreName)
	require.NoError(t, err)

	updater2, err := dataWallet2.IndexUpdater(rootIndex2)
	require.NoError(t, err)
	...
	f = sharedLocker.Share("bad-id", testbase.TestVaultName, expiry.Months(12))
	require.Error(t, f.Error())

	f = sharedLocker.Share(recordID, testbase.TestVaultName, expiry.Months(12))
...
func TestLocalStoreImpl_Revoke(t *testing.T) {
	...
	updater, err := dataWallet1.IndexUpdater(rootIndex1)
	require.NoError(t, err)
	defer updater.Close()
	...
	checkAccess := func(dw DataWallet, canAccess bool) {
		ds, err := dw.DataStore().Load(recordID)
		if canAccess {
			require.NoError(t, err)
		} else if err != nil {
			// operation has been purged
	...
	f = dataWallet2.DataStore().Revoke(recordID)
	require.Error(t, f.Wait(time.Second*10))

	// revocation of non-existent lease should fail
	badRecord, _ := utils.RandomID(32)
	f = dataWallet1.DataStore().Revoke(badRecord)
	require.Error(t, f.Wait(time.Second*10))

	// should succeed
	f = dataWallet1.DataStore().Revoke(recordID)
	require.NoError(t, f.Wait(time.Second*10))
	...
	err = dataWallet1.DataStore().PurgeDataAssets(recordID)
	require.NoError(t, err)
