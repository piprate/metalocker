...
// The LockerFunc type is an adapter to allow the use of ordinary
// function as Locker mutator.
type LockerFunc func(context.Context, *ent.LockerMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f LockerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.LockerMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.LockerMutation", m)
}
