...
// This example shows how a user can create multiple versions of a dataset.
//
...
func main() {
	...
	table.SetHeader([]string{"Record", "Asset", "Content Type", "Revision"})
	table.SetBorders(tablewriter.Border{Left: true, Top: false, Right: true, Bottom: false})
	table.SetCenterSeparator("|")

	table.Append([]string{
		future1.ID(),
		future1.Lease().Impression.Asset,
		future1.Lease().Impression.MetaResource.ContentType,
		fmt.Sprintf("%d", future1.Lease().Impression.Revision()),
	})
	table.Append([]string{
		future2.ID(),
		future2.Lease().Impression.Asset,
		future2.Lease().Impression.MetaResource.ContentType,
		fmt.Sprintf("%d", future2.Lease().Impression.Revision()),
	})
